...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($razao) || empty($nome) || empty($cnpj) || empty($inscricao) || empty($endereco) || empty($bairro) || 
        empty($numero) || empty($cidade)|| empty($estado)|| empty($cep)|| empty($email)|| empty($ramo) || empty($telefone)|| empty($senha)) {
        echo "Por favor, preencha todos os campos.";
    } 
    else{

        $sql = "INSERT INTO clientes_aprovacao (razao,nome,cnpj,inscricao,endereco,bairro,numero,cidade,estado,cep,email,ramo,telefone,senha) 
            VALUES ('$razao','$nome','$cnpj','$inscricao','$endereco','$bairro','$numero','$cidade','$estado','$cep','$email','$ramo','$telefone','$senha')";
